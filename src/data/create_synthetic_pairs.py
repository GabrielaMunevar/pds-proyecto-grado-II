...
def restructure_sentences(text: str, complexity_level: float) -> str:
    ...
    while i < len(sentences):
        ...
        if random.random() < complexity_level * 0.55 * base_multiplier:  # Aumentado a 0.55
            ...
            def make_passive(m):
                has_not = 'n\'t' in m.group(2) or 'don\'t' in m.group(2)
                not_str = 'not ' if has_not else ''
                return f"is {not_str}performed by {m.group(1)} "
            sentence = re.sub(
...
def load_data(data_path: str = None):
    ...
    if 'label' in df.columns:
        ...
        print("No se encontró columna 'label'. Asumiendo que registros con 'resumen' son PLS.")
        pls_data = df[df['resumen'].notna() & (df['resumen'].str.len() > 20)].copy()
    
    print(f"Total registros PLS: {len(pls_data)}")
    
    if len(pls_data) == 0:
        print("No se encontraron registros PLS. Verifica que el dataset tenga la columna 'label' o 'resumen'.")
...
def create_synthetic_pairs(pls_data, complexity_level: float = 0.8):
    ...
    # Umbrales de overlap léxico
    # Ajustados basados en análisis: mediana=0.704, 97% entre 0.60-0.80
    MIN_OVERLAP = 0.60  # demasiado diferente si es menor
    MAX_OVERLAP = 0.75  # demasiado parecido si es mayor
    
    total_rows = len(pls_data)
    print(f"Total registros a procesar: {total_rows:,}")
    print(f"Nivel de complejidad: {complexity_level:.2f}")
    print(f"Filtro de overlap léxico: {MIN_OVERLAP:.2f} - {MAX_OVERLAP:.2f}")
    print("\nProcesando...")
    ...
    progress_interval = max(1, total_rows // 20)  # 20 actualizaciones aproximadamente
    ...
    for idx, row in pls_data.iterrows():
        ...
        # Aceptar solo pares con solapamiento léxico moderado
        if lexical_overlap < MIN_OVERLAP or lexical_overlap > MAX_OVERLAP:
            # Descartar pares demasiado diferentes o demasiado similares
        ...
        if processed % progress_interval == 0 or processed == total_rows:
            ...
            print(f"  Progreso: {processed:,}/{total_rows:,} ({percentage:.1f}%) - Pares creados: {len(pairs):,}")
    
    print(f"\n✓ Procesamiento completado!")
    print(f"  - Pares sintéticos creados (tras filtro de overlap {MIN_OVERLAP:.2f}-{MAX_OVERLAP:.2f}): {len(pairs):,}")
    print(f"  - Registros omitidos (texto inválido): {skipped:,}")
    print(f"  - Pares descartados por overlap: {filtered_by_overlap:,}")
...
def main():
    ...
    if 'avg_lexical_overlap' in stats:
        ...
        if 0.50 <= stats['avg_lexical_overlap'] <= 0.65:
            ...
            print(f"   Overlap muy bajo - puede indicar transformaciones demasiado agresivas")
        elif stats['avg_lexical_overlap'] > 0.65:
            print(f"   Overlap alto - considerar ajustes para más diversidad")
